...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagHeater)
    {
        info->event[info->num] = EVENT_Heater;
        info->num++;
        dataPoints->valueHeater = gizStandardDecompressionValue(Heater_BYTEOFFSET,Heater_BITOFFSET,Heater_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagBubble)
    {
        info->event[info->num] = EVENT_Bubble;
        info->num++;
        dataPoints->valueBubble = gizStandardDecompressionValue(Bubble_BYTEOFFSET,Bubble_BITOFFSET,Bubble_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagFilter)
    {
        info->event[info->num] = EVENT_Filter;
        info->num++;
        dataPoints->valueFilter = gizStandardDecompressionValue(Filter_BYTEOFFSET,Filter_BITOFFSET,Filter_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagO3)
    {
        info->event[info->num] = EVENT_O3;
        info->num++;
        dataPoints->valueO3 = gizStandardDecompressionValue(O3_BYTEOFFSET,O3_BITOFFSET,O3_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagTemperature_setup)
    {
        info->event[info->num] = EVENT_Temperature_setup;
        info->num++;
        dataPoints->valueTemperature_setup = gizX2Y(Temperature_setup_RATIO,  Temperature_setup_ADDITION, issuedData->attrVals.valueTemperature_setup); 
    }
        
    if(0x01 == issuedData->attrFlags.flagTiming)
    {
        info->event[info->num] = EVENT_Timing;
        info->num++;
        dataPoints->valueTiming = gizX2Y(Timing_RATIO,  Timing_ADDITION, exchangeBytes(issuedData->attrVals.valueTiming));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueBubble != cur->valueBubble)
    {
        //GIZWITS_LOG("valueBubble Changed\n");
        ret = 1;
    }
    if(last->valueFilter != cur->valueFilter)
    {
        //GIZWITS_LOG("valueFilter Changed\n");
        ret = 1;
    }
    if(last->valueO3 != cur->valueO3)
    {
        //GIZWITS_LOG("valueO3 Changed\n");
        ret = 1;
    }
    if(last->valueTemperature_setup != cur->valueTemperature_setup)
    {
        //GIZWITS_LOG("valueTemperature_setup Changed\n");
        ret = 1;
    }
    if(last->valueTiming != cur->valueTiming)
    {
        //GIZWITS_LOG("valueTiming Changed\n");
        ret = 1;
    }
    if(last->valueOvertime_filter != cur->valueOvertime_filter)
    {
        //GIZWITS_LOG("valueOvertime_filter Changed\n");
        ret = 1;
    }
    if(last->valueSuperheat != cur->valueSuperheat)
    {
        //GIZWITS_LOG("valueSuperheat Changed\n");
        ret = 1;
    }
    if(last->valueUndercooling != cur->valueUndercooling)
    {
        //GIZWITS_LOG("valueUndercooling Changed\n");
        ret = 1;
    }

    if(last->valueTime_filter != cur->valueTime_filter)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            //GIZWITS_LOG("valueTime_filter Changed\n");
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Heater_BYTEOFFSET,Heater_BITOFFSET,Heater_LEN,(uint8_t *)devStatusPtr,dataPoints->valueHeater);
    gizStandardCompressValue(Bubble_BYTEOFFSET,Bubble_BITOFFSET,Bubble_LEN,(uint8_t *)devStatusPtr,dataPoints->valueBubble);
    gizStandardCompressValue(Filter_BYTEOFFSET,Filter_BITOFFSET,Filter_LEN,(uint8_t *)devStatusPtr,dataPoints->valueFilter);
    gizStandardCompressValue(O3_BYTEOFFSET,O3_BITOFFSET,O3_LEN,(uint8_t *)devStatusPtr,dataPoints->valueO3);
    gizStandardCompressValue(Overtime_filter_BYTEOFFSET,Overtime_filter_BITOFFSET,Overtime_filter_LEN,(uint8_t *)devStatusPtr,dataPoints->valueOvertime_filter);
    gizStandardCompressValue(Superheat_BYTEOFFSET,Superheat_BITOFFSET,Superheat_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSuperheat);
    gizStandardCompressValue(Undercooling_BYTEOFFSET,Undercooling_BITOFFSET,Undercooling_LEN,(uint8_t *)devStatusPtr,dataPoints->valueUndercooling);
    ...
    devStatusPtr->valueTemperature_setup = gizY2X(Temperature_setup_RATIO,  Temperature_setup_ADDITION, dataPoints->valueTemperature_setup); 

    devStatusPtr->valueTiming = exchangeBytes(gizY2X(Timing_RATIO,  Timing_ADDITION, dataPoints->valueTiming)); 
    devStatusPtr->valueTime_filter = exchangeBytes(gizY2X(Time_filter_RATIO,  Time_filter_ADDITION, dataPoints->valueTime_filter)); 
